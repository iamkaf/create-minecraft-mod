...
async function cloneTemplate(mod: Mod): Promise<void> {
  const s = spinner();
  s.start(`Cloning template for mod "${mod.name}"...`);
  try {
    await delay(1000);
...
async function addSampleCode(mod: Mod): Promise<void> {
  ...
  const s = spinner();
  s.start(`Adding sample code: ${mod.samples.join(', ')}`);
  try {
    await delay(800);
    s.stop(`Sample code added successfully`);
  } catch (error) {
    s.stop(`Failed to add sample code`, 1);
    throw new Error(`Sample code addition failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
async function installUtilityMods(mod: Mod): Promise<void> {
  ...
  const s = spinner();
  s.start(`Installing utility mods: ${mod.utility.join(', ')}`);
  try {
    await delay(600);
    s.stop(`Utility mods installed successfully`);
  } catch (error) {
    s.stop(`Failed to install utility mods`, 1);
    throw new Error(`Utility mod installation failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
async function installLibraries(mod: Mod): Promise<void> {
  ...
  const s = spinner();
  s.start(`Installing libraries: ${mod.libraries.join(', ')}`);
  try {
    await delay(700);
    s.stop(`Libraries installed successfully`);
  } catch (error) {
    s.stop(`Failed to install libraries`, 1);
    throw new Error(`Library installation failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
async function configureLoaders(mod: Mod): Promise<void> {
  ...
  try {
    ...
    throw new Error(`Loader configuration failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}

async function applyLicense(mod: Mod): Promise<void> {
  const s = spinner();
  s.start(`Applying license: ${mod.license}`);
  try {
    await delay(300);
    s.stop(`License applied successfully`);
  } catch (error) {
    s.stop(`Failed to apply license`, 1);
    throw new Error(`License application failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}

async function finalizeProject(mod: Mod): Promise<void> {
  const s = spinner();
  s.start(`Finalizing project structure for "${mod.name}"...`);
  try {
    await delay(900);
    s.stop(`Project finalized successfully`);
  } catch (error) {
    s.stop(`Failed to finalize project`, 1);
    throw new Error(`Project finalization failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}

async function runGradle(mod: Mod): Promise<void> {
  const s = spinner();
  s.start(`Running Gradle build for "${mod.name}"...`);
  try {
    await delay(2000);
    s.stop(`Gradle build completed`);
  } catch (error) {
    s.stop(`Failed to run Gradle build`, 1);
    throw new Error(`Gradle build failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
async function openInVSCode(mod: Mod): Promise<void> {
  const s = spinner();
  s.start(`Opening project in VS Code...`);
  try {
    await delay(200);
    s.stop(`VS Code opened`);
  } catch (error) {
    s.stop(`Failed to open VS Code`, 1);
    throw new Error(`VS Code opening failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}

async function openInIntelliJ(mod: Mod): Promise<void> {
  const s = spinner();
  s.start(`Opening project in IntelliJ IDEA...`);
  try {
    await delay(250);
    s.stop(`IntelliJ IDEA opened`);
  } catch (error) {
    s.stop(`Failed to open IntelliJ IDEA`, 1);
    throw new Error(`IntelliJ IDEA opening failed: ${error instanceof Error ? error.message : String(error)}`);
  }
}
